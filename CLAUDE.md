...
| **Containers** | **Currently ONE active: `data_ingest`** (UDP listener + OTE collector + MQTT→Loxone bridge; owns the `2000/udp` publish), built from `./loxone_smart_home/` running `main.py` with `GROWATT_CONTROLLER_ENABLED=false`. **Data collection is migrating to MPCHomeControl crate by crate** (2026-07): **weather** (`WEATHER_SCRAPER_ENABLED=false` here — `mpc-scraper-openmeteo` writes `weather_forecast`) and **Solcast** (the `solcast_writer` service is commented out — `mpc-scraper-solcast` writes `solar_forecast_history`) are now MPC-owned. `loxone_smart_home` (Growatt controller + API on 5556) and `loxone_web` (dashboard on 5555) are also **commented out in `docker-compose.yml`** — the controller is being **replaced by the MPC**. Uncomment the relevant block to revive any of these. The notes below describe the full three-container split for when the controller returns. |
