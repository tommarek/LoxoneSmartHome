...
def parse_arguments():
    ...
    date_group.add_argument(
        "--days", type=int, help="Number of days back from now to analyze"
    )
    date_group.add_argument(
        "--full", action="store_true", help="Run full 2-year analysis"
    )
    date_group.add_argument(
        "--date-range",
        nargs=2,
        ...
        if not skip_visualizations:
            ...
            try:
                ...
                visualizer.create_analysis_summary_report(
                    results, save_path=str(report_path)
                )
                ...
                if "data" in results:
                    ...
                    static_plots = visualizer.save_static_plots(
                        data, results, prefix="pems_analysis"
                    )
...
async def main():
    ...
    if duration.days > 1000:
        logger.warning(
            f"Large date range ({duration.days} days) - this may take a while"
        )
