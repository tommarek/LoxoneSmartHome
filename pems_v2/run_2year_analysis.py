...
async def run_2year_analysis():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('analysis_2year.log')
        ]
    )
    ...
    try:
        ...
        output_dirs = ["data/raw", "data/processed", "data/features", "analysis/results", "analysis/figures", "analysis/reports"]
        ...
        if not weather_data.empty:
            ...
            logger.info(f"  - Temperature range: {temp_stats['min']:.1f}°C to {temp_stats['max']:.1f}°C")
            ...
            try:
                ...
                for room_name, results in thermal_results.items():
                    ...
                    if isinstance(results, dict) and 'basic_stats' in results:
                        ...
                        logger.info(f"  {room_name:20s}: {mean_temp:5.1f}°C mean, {temp_range:4.1f}°C range")
                        room_count += 1
        ...
        if heating_data is not None and not heating_data.empty:
            power_stats = heating_data['total_heating_consumption'].describe()
            total_energy = (heating_data['total_heating_consumption'].sum() * 0.25) / 1000  # 15min intervals to kWh
            report_lines.extend([
        ...
        if room_data:
            ...
            for room_name, room_df in sorted_rooms[:15]:  # Top 15 rooms
                if not room_df.empty:
                    ...
                    report_lines.append(f"• {room_name:20s}: {len(room_df):6,} measurements, "
