...
class SystemValidator:
    ...
    async def validate_data_pipeline(self):
        ...
        try:
            ...
            data_summary = {
                ...
                'consumption_records': len(consumption_data) if consumption_data is not None else 0,
            ...
            if weather_data is not None and len(weather_data) > 0:
                processed_weather = preprocessor.process_dataset(weather_data, 'weather')
    ...
    async def validate_ml_models(self):
        ...
        try:
            ...
            if 'consumption' in datasets:
                ...
                if 'heating_power' in data.columns:
                    ...
                    if len(X) > 50:
                        ...
                        print(f'    ✅ Load model: MAE={metrics.mae:.1f}W, R²={metrics.r2:.3f}')
                    else:
                        model_results['load_predictor'] = {'status': 'insufficient_data'}
                        print(f'    ⚠️  Load model: insufficient data ({len(X)} samples)')
                else:
            ...
            if 'room' in datasets and 'outdoor' in datasets:
                ...
                merged = pd.merge(datasets['room'].tail(1000), datasets['outdoor'].tail(1000),
                ...
                if len(merged) > 50 and 'temperature' in merged.columns:
                    ...
                    if len(X) > 50:
                        ...
                        try:
                            ...
                            print(f'    ✅ Thermal model: MAE={metrics.mae:.2f}°C, R²={metrics.r2:.3f}')
                        except Exception as e:
                            model_results['thermal_predictor'] = {'status': 'training_failed', 'error': str(e)}
                            print(f'    ⚠️  Thermal model training failed: {e}')
                    else:
                        model_results['thermal_predictor'] = {'status': 'insufficient_data'}
                        print(f'    ⚠️  Thermal model: insufficient data')
                else:
                    model_results['thermal_predictor'] = {'status': 'no_data'}
                    print(f'    ⚠️  Thermal model: no merged data or temperature column')
    ...
    async def validate_optimization_engine(self):
        ...
        try:
            ...
            problem = create_optimization_problem(
                ...
                horizon_hours=6  # Shorter for validation
            )
            ...
            if result.success:
                total_heating_hours = sum(schedule.sum() * 0.25 for schedule in result.heating_schedule.values())
    ...
    async def validate_control_interfaces(self):
        ...
        try:
            ...
            schedule = {
                'obyvak': pd.Series([True, False, True], 
                                  index=pd.date_range(start=datetime.now(), periods=3, freq='15min')),
                'kuchyne': pd.Series([False, True, False],
                                   index=pd.date_range(start=datetime.now(), periods=3, freq='15min'))
            }
            ...
            control_results = {
                ...
                'schedule_success_rate': sum(schedule_results.values()) / len(schedule_results),
            ...
            print(f'       Success rate: {control_results["schedule_success_rate"]*100:.1f}%')
    ...
    async def validate_end_to_end_workflow(self):
        ...
        try:
            ...
            for name, file_path in data_files.items():
                if os.path.exists(file_path):
                    datasets[name] = pd.read_parquet(file_path).tail(100)  # Last 100 records
            ...
            price_forecast = pd.Series([0.15 if 7 <= h <= 22 else 0.08 for h in range(24)], index=time_index)
            ...
            if result.success:
                ...
                for room, schedule in result.heating_schedule.items():
                    first_hour_schedule[room] = schedule.head(4)  # First 4 intervals (1 hour)
                
                control_results = await heating_controller.execute_schedule(first_hour_schedule)
    ...
    async def validate_performance(self):
        ...
        try:
            ...
            if os.path.exists('data/processed/pv_processed.parquet'):
                ...
                print(f'    ✅ Loaded {len(data):,} records in {load_time:.3f}s ({len(data)/load_time:.0f} rec/s)')
            
            # 2. Optimization benchmark
            print('  🧮 Optimization benchmark...')
            config = {'rooms': {'test': {'power_kw': 1.0}}, 'battery': {'capacity_kwh': 10.0}}
            optimizer = EnergyOptimizer(config)
    ...
    def generate_validation_report(self):
        ...
        if 'performance' in self.results:
            ...
            if 'data_loading' in perf:
                ...
                print(f'   Data loading rate: {data_perf["records_per_second"]:.0f} records/s')
