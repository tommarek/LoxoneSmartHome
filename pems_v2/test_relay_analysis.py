...
class RelayAnalyzer:
        ...
        for table in tables:
            for record in table.records:
                ...
                if room in self.heating_power:
                    relay_state = record.get_value()  # 0.0 = OFF, 1.0 = ON (or values in between for partial)
                    ...
                    relay_data.append({
                        ...
                        'actual_power_w': relay_state * self.heating_power[room] * 1000,  # Watts when ON
                        'energy_kwh_15min': (relay_state * self.heating_power[room] * 0.25)  # Energy per 15min interval
                    })
    ...
    def analyze_relay_patterns(self, relay_df: pd.DataFrame) -> dict:
        ...
        results['summary'] = {
            ...
            'system_utilization_percent': (total_energy / (total_capacity * analysis_hours)) * 100 if analysis_hours > 0 else 0
        }
        ...
        for room in relay_df['room'].unique():
            ...
            results['rooms'][room] = {
                ...
                'energy_efficiency_ratio': room_energy / (self.heating_power[room] * analysis_hours) if analysis_hours > 0 else 0
            }
    ...
    def print_analysis_report(self, analysis_results: dict):
        ...
        print("-" * 80)
        print(f"{'Room':<20} {'Duty%':<8} {'Switches':<10} {'Energy(kWh)':<12} {'Rating(kW)':<12} {'Efficiency':<10}")
        print("-" * 80)
        ...
        top_consumers = sorted(rooms.items(), key=lambda x: x[1]['total_energy_kwh'], reverse=True)[:3]
        print(f"Top Energy Consumers:")
        for room, stats in top_consumers:
            print(f"  {room}: {stats['total_energy_kwh']:.1f} kWh ({stats['duty_cycle_percent']:.1f}% duty cycle)")
        
        # Most active relays (highest switching)
        active_relays = sorted(rooms.items(), key=lambda x: x[1]['total_switches'], reverse=True)[:3]
        print(f"\nMost Active Relays:")
        for room, stats in active_relays:
            print(f"  {room}: {stats['total_switches']:.0f} switches (avg {stats['avg_on_minutes']:.1f}min on, {stats['avg_off_minutes']:.1f}min off)")
