...
class BaseLoadAnalyzer:
    ...
    def analyze_base_load(self, 
                         consumption_data: pd.DataFrame, 
                         pv_data: pd.DataFrame,
                         room_data: Dict[str, pd.DataFrame],
                         ev_data: Optional[pd.DataFrame] = None,
                         battery_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """
    ...
    def _calculate_base_load(self, 
                           ...
                           battery_data: Optional[pd.DataFrame]) -> pd.DataFrame:
        """Calculate base load by subtracting controllable loads."""
        
        # Find total consumption column
        consumption_cols = [col for col in consumption_data.columns 
                          if any(keyword in col.lower() for keyword in ['consumption', 'total', 'load', 'grid_import'])]
        ...
        if not pv_data.empty:
            ...
            if pv_power_cols:
                ...
                pv_self_consumption = np.minimum(pv_aligned.fillna(0), base_load_data['total_consumption'])
        ...
        if ev_data is not None and not ev_data.empty:
            ev_cols = [col for col in ev_data.columns if any(keyword in col.lower() for keyword in ['power', 'charge'])]
            if ev_cols:
        ...
        if battery_data is not None and not battery_data.empty:
            ...
            if battery_cols:
                ...
                battery_resampled = battery_power.resample(total_consumption.index.freq or '15T').mean()
    ...
    def _estimate_heating_consumption(self, room_data: Dict[str, pd.DataFrame], target_index: pd.DatetimeIndex) -> Optional[pd.Series]:
        ...
        for room_name, room_df in room_data.items():
            ...
            heating_cols = [col for col in room_df.columns 
    ...
    def _calculate_base_load_stats(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'total_consumption' in base_load_data.columns:
            ...
            stats['base_load_percentage'] = (base_load.sum() / total_consumption.sum() * 100) if total_consumption.sum() > 0 else 0
            
            # Breakdown of other components
            components = ['pv_self_consumption', 'heating_consumption', 'ev_consumption', 'battery_consumption']
            for component in components:
                if component in base_load_data.columns:
                    component_sum = base_load_data[component].sum()
                    stats[f'{component}_percentage'] = (component_sum / total_consumption.sum() * 100) if total_consumption.sum() > 0 else 0
        ...
        stats.update({
            ...
            'peak_to_minimum_ratio': hourly_avg.max() / hourly_avg.min() if hourly_avg.min() > 0 else float('inf')
        })
    ...
    def _analyze_time_patterns(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        patterns['weekday_vs_weekend'] = {
            ...
            'weekend_increase': (weekend_load.mean() / weekday_load.mean() - 1) * 100 if weekday_load.mean() > 0 else 0,
    ...
    def _analyze_seasonal_patterns(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            seasonal_analysis = {
                ...
                'has_strong_seasonal': 1 - (result.resid.var() / (result.seasonal + result.resid).var()) > 0.6,
                'has_strong_trend': 1 - (result.resid.var() / (result.trend + result.resid).var()) > 0.6
            }
            ...
            if seasonal_profiles:
                highest_season = max(seasonal_profiles.keys(), key=lambda s: seasonal_profiles[s]['mean_load'])
                lowest_season = min(seasonal_profiles.keys(), key=lambda s: seasonal_profiles[s]['mean_load'])
                ...
                seasonal_analysis.update({
                    ...
                    'seasonal_variation_percentage': (
                        (seasonal_profiles[highest_season]['mean_load'] / seasonal_profiles[lowest_season]['mean_load'] - 1) * 100
                        if seasonal_profiles[lowest_season]['mean_load'] > 0 else 0
                    )
                })
    ...
    def _analyze_load_profiles(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(daily_profiles) < 30:  # Need at least 30 days
            return {'warning': 'Insufficient data for load profile analysis (need at least 30 complete days)'}
    ...
    def _analyze_energy_efficiency(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        efficiency_metrics = {
            ...
            'consumption_variability': {
                'hourly_cv': hourly_consumption.std() / hourly_consumption.mean() if hourly_consumption.mean() > 0 else 0,
                'daily_cv': daily_consumption.std() / daily_consumption.mean() if daily_consumption.mean() > 0 else 0,
                'monthly_cv': monthly_consumption.std() / monthly_consumption.mean() if monthly_consumption.mean() > 0 else 0
            }
        }
        ...
        if len(daily_consumption) > 30:
            ...
            efficiency_metrics.update({
                ...
                'efficiency_ratio': least_efficient_days.mean() / most_efficient_days.mean() if most_efficient_days.mean() > 0 else 0
            })
        ...
        if not night_consumption.empty:
            efficiency_metrics['nighttime_efficiency'] = {
                ...
                'night_load_factor': night_consumption.mean() / base_load.mean() if base_load.mean() > 0 else 0
            }
    ...
    def _detect_special_events(self, base_load_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'high_consumption_events': {
                ...
                'avg_consumption': high_consumption_events.mean() if not high_consumption_events.empty else None
            },
            'low_consumption_events': {
                'count': len(low_consumption_events),
                'sample_dates': low_consumption_events.index[:10].tolist(),
                'avg_consumption': low_consumption_events.mean() if not low_consumption_events.empty else None
            },
