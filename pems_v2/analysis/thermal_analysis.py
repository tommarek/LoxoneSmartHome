...
class ThermalAnalyzer:
    ...
    def analyze_room_dynamics(self, room_data: Dict[str, pd.DataFrame], weather_data: pd.DataFrame) -> Dict[str, Any]:
        """
    ...
    def _analyze_single_room(self, room_df: pd.DataFrame, weather_data: pd.DataFrame, room_name: str) -> Dict[str, Any]:
    ...
    def _merge_room_weather_data(self, room_df: pd.DataFrame, weather_data: pd.DataFrame) -> pd.DataFrame:
        ...
        heating_cols = [col for col in room_df.columns if 'heating' in col.lower() or 'heat' in col.lower()]
        if heating_cols:
        ...
        if not weather_data.empty and 'temperature' in weather_data.columns:
            weather_resampled = weather_data[['temperature']].resample('5T').interpolate(method='linear')
    ...
    def _calculate_basic_thermal_stats(self, data: pd.DataFrame, room_name: str) -> Dict[str, Any]:
        ...
        if 'heating_on' in data.columns:
            ...
            stats.update({
                ...
                'mean_temp_heating_on': heating_data['room_temp'].mean() if not heating_data.empty else None,
                'mean_temp_heating_off': data[data['heating_on'] == 0]['room_temp'].mean() if (data['heating_on'] == 0).any() else None
            })
    ...
    def _analyze_heatup_cooldown(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        data_copy = data.copy()
        data_copy['temp_change_rate'] = data_copy['room_temp'].diff() * 12  # Per hour (5-min intervals)
        ...
        results = {
            'heatup_rate': {
                'mean_rate': heatup_data['temp_change_rate'].mean() if not heatup_data.empty else None,
                'max_rate': heatup_data['temp_change_rate'].max() if not heatup_data.empty else None,
                'std_rate': heatup_data['temp_change_rate'].std() if not heatup_data.empty else None,
                'samples': len(heatup_data)
            },
            'cooldown_rate': {
                'mean_rate': abs(cooldown_data['temp_change_rate'].mean()) if not cooldown_data.empty else None,
                'max_rate': abs(cooldown_data['temp_change_rate'].min()) if not cooldown_data.empty else None,
                'std_rate': cooldown_data['temp_change_rate'].std() if not cooldown_data.empty else None,
        ...
        if not heatup_data.empty and 'outdoor_temp' in data.columns:
            ...
            if len(temp_diff_heatup) > 10:
                ...
                slope, intercept, r_value, p_value, std_err = linregress(temp_diff_heatup, heatup_rate_values)
    ...
    def _calculate_heat_loss_coefficient(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        cooling_data = stable_data[stable_data['temp_change_rate'] < -0.05]  # At least 0.05°C/hour cooling
    ...
    def _analyze_solar_gains(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        data_copy = data.copy()
        data_copy['solar_proxy'] = np.sin(2 * np.pi * data_copy['hour'] / 24) * np.sin(2 * np.pi * data_copy.index.dayofyear / 365)
        ...
        peak_solar_hours = no_heating_data[(no_heating_data['hour'] >= 11) & (no_heating_data['hour'] <= 15)]
    ...
    def _analyze_thermal_comfort(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        stability_stats = {
            ...
            'stable_hours_percentage': (hourly_std < 0.5).mean() * 100  # Hours with <0.5°C variation
        }
    ...
    def _analyze_room_coupling(self, room_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        correlations = [(pair, data['correlation']) for pair, data in coupling_results['room_pairs'].items()]
        if correlations:
