...
class ThermalPreprocessingVisualizer:
    ...
    def visualize_room_preprocessing(
        ...
        if date_range:
            ...
            if weather_data is not None:
                mask = (weather_data.index >= start_date) & (weather_data.index <= end_date)
        ...
        ax1.set_title(f'{room_name} - Temperature Data Processing', fontsize=14, fontweight='bold')
        
        # Plot 2: Heating States and Temperature
        ax2 = fig.add_subplot(gs[1], sharex=ax1)
        self._plot_heating_states(ax2, processed_data, relay_data)
        ax2.set_title('Heating States and Temperature Response', fontsize=14, fontweight='bold')
        
        # Plot 3: Temperature Derivatives
        ax3 = fig.add_subplot(gs[2], sharex=ax1)
        self._plot_temperature_derivatives(ax3, processed_data)
        ax3.set_title('Temperature Rate of Change (dT/dt)', fontsize=14, fontweight='bold')
        
        # Plot 4: Identified Cycles
        ax4 = fig.add_subplot(gs[3], sharex=ax1)
        self._plot_identified_cycles(ax4, processed_data, identified_cycles, relay_data)
        ax4.set_title('Identified Heating and Decay Cycles', fontsize=14, fontweight='bold')
        
        # Plot 5: Cycle Statistics
        ax5 = fig.add_subplot(gs[4])
        self._plot_cycle_statistics(ax5, identified_cycles)
        ax5.set_title('Cycle Acceptance/Rejection Statistics', fontsize=14, fontweight='bold')
        ...
        # Save figure
        filename = f"preprocessing_{room_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    def _plot_temperature_comparison(
        ...
        if 'room_temp' in raw_data.columns:
            ax.plot(raw_data.index, raw_data['room_temp'], 
                   color=self.colors['raw_temp'], alpha=0.5, linewidth=1,
                   label='Raw Temperature')
        
        # Plot processed temperature
        if 'room_temp' in processed_data.columns:
            ax.plot(processed_data.index, processed_data['room_temp'],
                   color=self.colors['processed_temp'], linewidth=2,
        ...
        if weather_data is not None and 'outdoor_temp' in weather_data.columns:
            ax2 = ax.twinx()
            ax2.plot(weather_data.index, weather_data['outdoor_temp'],
                    color=self.colors['outdoor_temp'], linewidth=1.5, alpha=0.7,
                    label='Outdoor Temperature')
            ax2.set_ylabel('Outdoor Temperature (°C)', color=self.colors['outdoor_temp'])
    ...
    def _plot_heating_states(
        ...
        if relay_data is not None and 'state' in relay_data.columns:
            ...
            for start, end in zip(heating_starts, heating_ends):
                ax.axvspan(start, end, alpha=0.3, color=self.colors['heating_on'],
    ...
    def _plot_temperature_derivatives(
        self,
        ax: plt.Axes,
        processed_data: pd.DataFrame
    ):
        ...
        if 'room_temp' in processed_data.columns:
            ...
            # Plot derivative
            ax.plot(processed_data.index, dT_dt_smooth,
                   color='black', linewidth=1.5, label='dT/dt')
            
            # Add zero line
            ax.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
            
            # Shade positive (heating) and negative (cooling) regions
            ax.fill_between(processed_data.index, 0, dT_dt_smooth,
                           where=(dT_dt_smooth > 0), interpolate=True,
                           alpha=0.3, color=self.colors['heating_on'],
                           label='Temperature Rising')
            ax.fill_between(processed_data.index, 0, dT_dt_smooth,
                           where=(dT_dt_smooth < 0), interpolate=True,
                           alpha=0.3, color=self.colors['heating_off'],
    ...
    def _plot_identified_cycles(
        ...
        if 'room_temp' in processed_data.columns:
            ax.plot(processed_data.index, processed_data['room_temp'],
                   color='gray', linewidth=1, alpha=0.5, label='Temperature')
        
        if identified_cycles:
            # Plot heating cycles
            if 'heating' in identified_cycles:
                for cycle in identified_cycles['heating']:
                    color = (self.colors['accepted_rise'] if cycle.get('accepted', True)
                            else self.colors['rejected_rise'])
                    alpha = 0.6 if cycle.get('accepted', True) else 0.3
                    
                    ax.axvspan(cycle['start_time'], cycle['end_time'],
                             alpha=alpha, color=color)
                    
                    # Add rejection reason if rejected
                    if not cycle.get('accepted', True):
                        mid_time = cycle['start_time'] + (cycle['end_time'] - cycle['start_time']) / 2
            ...
            if 'decay' in identified_cycles:
                for cycle in identified_cycles['decay']:
                    color = (self.colors['accepted_decay'] if cycle.get('accepted', True)
                    ...
                    if not cycle.get('accepted', True):
                        mid_time = cycle['start_time'] + (cycle['end_time'] - cycle['start_time']) / 2
                        ax.text(mid_time, ax.get_ylim()[0] * 1.05,
                               cycle.get('rejection_reason', 'Unknown'),
                               rotation=90, ha='center', va='bottom', fontsize=8)
        
        # Create legend
        legend_elements = [
            mpatches.Patch(color=self.colors['accepted_rise'], alpha=0.6, label='Accepted Heating'),
            mpatches.Patch(color=self.colors['rejected_rise'], alpha=0.3, label='Rejected Heating'),
            mpatches.Patch(color=self.colors['accepted_decay'], alpha=0.6, label='Accepted Decay'),
            mpatches.Patch(color=self.colors['rejected_decay'], alpha=0.3, label='Rejected Decay'),
        ]
    ...
    def _plot_cycle_statistics(
        ...
        if 'heating' in identified_cycles:
            for cycle in identified_cycles['heating']:
                if cycle.get('accepted', True):
                    ...
                    rejection_reasons['heating'][reason] = rejection_reasons['heating'].get(reason, 0) + 1
        ...
        if 'decay' in identified_cycles:
            for cycle in identified_cycles['decay']:
                if cycle.get('accepted', True):
                    ...
                    reason = cycle.get('rejection_reason', 'Unknown')
                    rejection_reasons['decay'][reason] = rejection_reasons['decay'].get(reason, 0) + 1
        ...
        bars1 = ax.bar(x - width/2, accepted, width, label='Accepted', color='green', alpha=0.7)
        bars2 = ax.bar(x + width/2, rejected, width, label='Rejected', color='red', alpha=0.7)
        ...
        for cycle_type in ['heating', 'decay']:
            if rejection_reasons[cycle_type]:
                ...
                for reason, count in sorted(rejection_reasons[cycle_type].items(), 
                                          key=lambda x: x[1], reverse=True):
                    text_lines.append(f"  {reason}: {count}")
        
        ax.text(0.02, 0.98, '\n'.join(text_lines), transform=ax.transAxes,
               fontsize=10, va='top', ha='left', bbox=dict(boxstyle='round', 
    ...
    def _highlight_preprocessing_changes(
        ...
        if 'room_temp' not in raw_data.columns or 'room_temp' not in processed_data.columns:
            return
        ...
        if outlier_mask.any():
            ...
            ax.scatter(outlier_times, outlier_values, 
                      color=self.colors['outlier'], s=50, alpha=0.7,
                      marker='x', label='Outliers Removed')
        ...
        if interpolated_mask.any():
            ...
            for i, (time, is_interpolated) in enumerate(zip(common_index, interpolated_mask)):
                if is_interpolated:
            ...
            for start, end in interpolated_groups:
                ax.axvspan(start, end, alpha=0.2, color=self.colors['interpolated'],
...
def create_winter_analysis_summary(
    ...
    all_time_constants = []
    for result in analysis_results.values():
        if 'thermal_parameters' in result and 'time_constant_hours' in result['thermal_parameters']:
    ...
    if all_time_constants:
        ...
        ax.axvline(np.median(all_time_constants), color='red', linestyle='--',
    ...
    successful_rooms = sum(1 for r in analysis_results.values() 
                          if 'thermal_parameters' in r and r['thermal_parameters'])
    total_cycles = sum(r.get('total_cycles_analyzed', 0) for r in analysis_results.values())
    accepted_cycles = sum(r.get('accepted_cycles', 0) for r in analysis_results.values())
