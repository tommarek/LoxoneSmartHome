...
class PVAnalyzer:
    ...
    def analyze_pv_production(self, pv_data: pd.DataFrame, weather_data: pd.DataFrame) -> Dict[str, Any]:
        """
    ...
    def _merge_pv_weather_data(self, pv_data: pd.DataFrame, weather_data: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_solar_elevation(self, timestamps: pd.DatetimeIndex, latitude: float) -> pd.Series:
    ...
    def _calculate_basic_statistics(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'daylight_capacity_factor': daylight_data.mean() / pv_power.max() if pv_power.max() > 0 and len(daylight_data) > 0 else 0,
    ...
    def _analyze_weather_correlations(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        weather_cols = [col for col in data.columns if col not in ['pv_power', 'hour', 'day_of_year', 'month', 'weekday']]
        ...
        for col in weather_cols:
            if col in data.columns and data[col].notna().sum() > 100:  # Minimum data points
                ...
                correlations[col] = {
                    'correlation': corr,
                    'p_value': stats.pearsonr(data['pv_power'].dropna(), data[col].dropna())[1] if len(data[col].dropna()) > 10 else 1.0,
        ...
        return {
            ...
            'significant_correlations': {k: v for k, v in correlations.items() if abs(v['correlation']) > 0.3 and v['p_value'] < 0.05}
        }
    ...
    def _analyze_seasonal_patterns(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Monthly profiles
            monthly_profiles = data.groupby([data.index.month, data.index.hour])['pv_power'].mean().unstack(level=0)
            ...
            for season, months in seasons.items():
                ...
                if not season_data.empty:
                    seasonal_profiles[season] = season_data.groupby(season_data.index.hour)['pv_power'].mean()
            
            return {
                'decomposition': seasonal_patterns,
                'monthly_profiles': monthly_profiles.to_dict(),
                'seasonal_profiles': seasonal_profiles,
                'peak_season': max(seasons.keys(), key=lambda s: data[data.index.month.isin(seasons[s])]['pv_power'].mean()) if data['pv_power'].sum() > 0 else None
            }
    ...
    def _analyze_clear_sky_conditions(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        clear_sky_max = clear_sky_data.groupby([clear_sky_data.index.month, clear_sky_data.index.hour])['pv_power'].quantile(0.95)
        ...
        analysis = {
            ...
            'clear_sky_advantage': (clear_sky_data['pv_power'].mean() / data[~clear_sky_mask]['pv_power'].mean()) if data[~clear_sky_mask]['pv_power'].mean() > 0 else float('inf')
        }
    ...
    def _analyze_efficiency_patterns(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'temperature' not in data.columns or 'solar_elevation' not in data.columns:
            return {'warning': 'Insufficient data for efficiency analysis (need temperature and solar elevation)'}
        ...
        for low, high in temp_ranges:
            temp_mask = (meaningful_data['temperature'] >= low) & (meaningful_data['temperature'] < high)
        ...
        return {
            ...
            'optimal_temperature_range': min(temp_efficiency.items(), key=lambda x: x[1]['mean_power'])[0] if temp_efficiency else None
        }
    ...
    def _detect_anomalies(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        anomalies = daylight_data[(daylight_data['pv_power'] < lower_bound) | (daylight_data['pv_power'] > upper_bound)]
