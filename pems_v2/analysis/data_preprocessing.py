...
class DataValidator:
    ...
    def validate_pv_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in power_cols:
            if col in df.columns:
                ...
                if max_power > 50000:  # 50kW seems high for residential
                    validation_results['warnings'].append(f"Very high maximum power in {col}: {max_power}W")
                
                negative_count = (df[col] < 0).sum()
                if negative_count > 0:
                    validation_results['warnings'].append(f"Found {negative_count} negative power values in {col}")
        ...
        if len(df) > 1:
            ...
            if len(large_gaps) > 0:
                validation_results['warnings'].append(f"Found {len(large_gaps)} time gaps larger than 2 hours")
    ...
    def validate_temperature_data(self, df: pd.DataFrame, room_name: str) -> Dict[str, Any]:
        ...
        temp_cols = [col for col in df.columns if any(keyword in col.lower() for keyword in ['temp', 'temperature'])]
        if not temp_cols:
        ...
        for col in temp_cols:
            if col in df.columns:
                ...
                if df[col].nunique() < 5:
                    validation_results['warnings'].append(f"Very few unique temperature values in {room_name}.{col}")
...
class OutlierDetector:
    ...
    def detect_statistical_outliers(self, series: pd.Series, method: str = 'iqr') -> pd.Series:
        ...
        if method == 'iqr':
            ...
            return pd.Series(z_scores > 3, index=series.index).reindex(series.index, fill_value=False)
    ...
    def detect_contextual_outliers(self, series: pd.Series, context_window: str = '24H') -> pd.Series:
...
class GapFiller:
    ...
    def fill_gaps(self, df: pd.DataFrame, method: str = 'adaptive', max_gap_hours: int = 6) -> pd.DataFrame:
        ...
        for group_id in missing_groups[is_missing].unique():
            ...
            if gap_hours <= max_gap_hours:
                ...
                if gap_hours <= 1:  # Short gaps: linear interpolation
                    filled_series.loc[gap_mask] = series.loc[gap_start:gap_end].interpolate(method='linear')
                elif gap_hours <= 3:  # Medium gaps: spline interpolation
                    filled_series.loc[gap_mask] = series.loc[gap_start:gap_end].interpolate(method='spline', order=2)
    ...
    def _seasonal_fill(self, series: pd.Series, max_gap_hours: int) -> pd.Series:
        ...
        weekday_hour_pattern = series_reindexed.groupby([
            series_reindexed.index.weekday,
            series_reindexed.index.hour
        ]).median()
...
class FeatureEngineer:
    ...
    def create_rolling_features(self, df: pd.DataFrame, target_col: str, windows: List[int]) -> pd.DataFrame:
        ...
        for window in windows:
            ...
            features_df[f'{target_col}_rolling_std_{window}'] = features_df[target_col].rolling(window=window, min_periods=1).std()
