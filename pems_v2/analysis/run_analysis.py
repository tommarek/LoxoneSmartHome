...
class DataPreprocessor:
    ...
    def process_dataset(self, df: pd.DataFrame, data_type: str) -> pd.DataFrame:
        ...
        self.logger.info(f"Processed {data_type}: {len(df_clean)} clean records from {len(df)} original")
        return df_clean
    ...
    def _generate_quality_report(self, original_df: pd.DataFrame, clean_df: pd.DataFrame, data_type: str) -> Dict[str, Any]:
        ...
        original_missing_pct = (original_missing / original_total * 100) if original_total > 0 else 100
        ...
        return {
            ...
            'date_range': (original_df.index.min(), original_df.index.max()) if not original_df.index.empty else (None, None),
...
class AnalysisVisualizer:
    ...
    def create_summary_report(self, analysis_results: Dict[str, Any]) -> str:
        ...
        if 'pv_analysis' in analysis_results:
            ...
            if 'weather_correlations' in pv_results and 'strongest_positive' in pv_results['weather_correlations']:
                strongest = pv_results['weather_correlations']['strongest_positive']
                if strongest:
                    report_lines.append(f"• Strongest Weather Correlation: {strongest[0]} ({strongest[1]['correlation']:.3f})")
        ...
        if 'thermal_analysis' in analysis_results:
            ...
            for room_name, room_data in thermal_results.items():
                ...
                if isinstance(room_data, dict) and 'basic_stats' in room_data:
                    ...
                    if 'time_constant' in room_data and isinstance(room_data['time_constant'], dict):
                        tc = room_data['time_constant']
                        if 'time_constant_hours' in tc:
                            report_lines.append(f"  • Time Constant: {tc['time_constant_hours']:.1f} hours")
        ...
        if 'base_load_analysis' in analysis_results:
            ...
            if 'time_patterns' in base_load_results and 'weekday_vs_weekend' in base_load_results['time_patterns']:
                pattern = base_load_results['time_patterns']['weekday_vs_weekend']
                report_lines.append(f"• Weekend vs Weekday: {pattern.get('weekend_increase', 0):.1f}% higher on weekends")
...
class AnalysisPipeline:
        ...
        try:
            ...
            for room_name, room_df in data['rooms'].items():
                if not room_df.empty:
                    ...
                    processed['rooms'][room_name] = self.preprocessor.process_dataset(room_df, 'temperature')
                else:
            ...
            if not data['consumption'].empty:
                ...
                processed['consumption'] = self.preprocessor.process_dataset(data['consumption'], 'consumption')
            else:
            ...
            for data_type, report in self.preprocessor.quality_report.items():
                ...
                self.logger.info(f"  Records: {report['total_records']} -> {report['clean_records']}")
                if report['date_range'][0] and report['date_range'][1]:
                    self.logger.info(f"  Date range: {report['date_range'][0].strftime('%Y-%m-%d')} to {report['date_range'][1].strftime('%Y-%m-%d')}")
                self.logger.info(f"  Missing data: {report['original_missing_percentage']}% -> {report['clean_missing_percentage']}%")
    ...
    async def _run_all_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results = {
            'data_quality': self.preprocessor.quality_report
        }
            ...
            for data_type, df in data.items():
                if isinstance(df, pd.DataFrame) and not df.empty:
                    ...
                    for room_name, room_df in df.items():
                        if not room_df.empty:
                            filepath = Path(f"data/processed/{data_type}_{room_name}_processed.parquet")
                            room_df.to_parquet(filepath)
                            self.logger.info(f"Saved processed {data_type} {room_name} data to {filepath}")
