...
def sample_weather_data():
    ...
    dates = pd.date_range("2024-01-01 00:00:00", "2024-01-30 23:00:00", freq="1h")
...
def sample_pv_data():
    ...
    # Use enough days to cover the longest lag (prev_week = 288*7 = 2016 periods)
    # For hourly data, need at least 8+ days to cover weekly lag
    dates = pd.date_range("2024-01-01 00:00:00", "2024-01-30 23:00:00", freq="1h")
...
class TestPVPredictor:
    ...
    def test_weather_feature_creation(self, pv_config, sample_weather_data):
        ...
        ) as mock_solar_pos, patch.object(
            ...
            mock_solar_pos.return_value = pd.DataFrame(
                {
                    ...
                    "apparent_zenith": np.random.uniform(
                        0, 90, len(sample_weather_data)
                    ),
                },
    ...
    def test_temporal_feature_creation(self, pv_config, sample_weather_data):
        ...
        temporal_features = predictor.create_temporal_features(
            sample_weather_data.index
        )
    ...
    def test_physical_model_calculation(
        ...
        with patch.object(predictor.location, "get_solarposition") as mock_solar_pos:
            ...
            physical_prediction = predictor.calculate_physical_model_baseline(
                sample_weather_data
            )
    ...
    def test_training_data_preparation(
        self, pv_config, sample_pv_data, sample_weather_data
    ):
        """Test training data preparation."""
        predictor = PVPredictor(pv_config)

        # Mock physical model calculation and lag features for hourly data
        with patch.object(
            predictor, "calculate_physical_model_baseline"
        ) as mock_physical, patch.object(
            predictor, "create_lag_features"
        ) as mock_lag_features:
            mock_physical.return_value = pd.Series(
                np.random.uniform(0, 10, len(sample_pv_data)),
                index=sample_pv_data.index,
            )

            # Mock lag features to avoid the 5-minute assumption for test data
            def mock_lag_func(data, target_col):
                features = data.copy()
                # Add simple hourly-appropriate lag features
                features[f"{target_col}_lag_1"] = features[target_col].shift(1)
                features[f"{target_col}_lag_2"] = features[target_col].shift(2)
                features[f"{target_col}_lag_3"] = features[target_col].shift(3)
                features[f"{target_col}_lag_6"] = features[target_col].shift(6)
                features[f"{target_col}_lag_12"] = features[target_col].shift(12)
                features[f"{target_col}_lag_24"] = features[target_col].shift(24)
                features[f"{target_col}_rolling_mean_6"] = (
                    features[target_col].rolling(6).mean()
                )
                features[f"{target_col}_rolling_std_6"] = (
                    features[target_col].rolling(6).std()
                )
                features[f"{target_col}_rolling_mean_12"] = (
                    features[target_col].rolling(12).mean()
                )
                features[f"{target_col}_rolling_std_12"] = (
                    features[target_col].rolling(12).std()
                )
                features[f"{target_col}_rolling_mean_24"] = (
                    features[target_col].rolling(24).mean()
                )
                features[f"{target_col}_rolling_std_24"] = (
                    features[target_col].rolling(24).std()
                )
                features[f"{target_col}_prev_day"] = features[target_col].shift(24)
                features[f"{target_col}_prev_week"] = features[target_col].shift(24 * 7)
                return features

            mock_lag_features.side_effect = mock_lag_func
            ...
            with patch.object(
                ...
                X, y = predictor.prepare_training_data(
                    sample_pv_data, sample_weather_data
                )
    ...
    def test_prediction_clipping(self, pv_config):
        ...
        predictor.ml_model.predict.return_value = np.array(
            [15.0]
        )  # 15 kW > 10 kW capacity

        # Create simple test data
        X_test = pd.DataFrame(
            {"test_feature": [1.0]}, index=[pd.Timestamp("2024-01-01 12:00:00")]
        )
        ...
        assert (
            result.predictions.iloc[0] <= predictor.capacity_kw * 1000
        )  # Should be <= 10,000 W
