...
from pems_v2.models.base import (BasePredictor, ModelMetadata, ModelRegistry,
                                 PerformanceMetrics, PredictionResult)


class SimplePicklableModel:
    """Simple model mock that can be pickled."""
    ...
    def predict(self, X):
        # Return predictions matching the input length
...
class MockPredictor(BasePredictor):
    ...
    def train(self, X, y, validation_data=None, **kwargs):
        ...
        self.model = SimplePicklableModel(y.values)
        ...
        # Store latest performance in metadata for test_model_summary
    ...
    def predict(self, X, return_uncertainty=False, **kwargs):
        ...
        # Generate predictions of the right length for input X
        prediction_values = np.random.normal(0, 1, len(X))
        predictions = pd.Series(prediction_values, index=X.index, name="predictions")
...
class TestBasePredictor:
    ...
    def test_model_evaluation(self, sample_config, sample_data):
        ...
        assert isinstance(performance, PerformanceMetrics)
        # Should have 2 entries: one from training, one from evaluation
        assert len(predictor.performance_history) == 2
