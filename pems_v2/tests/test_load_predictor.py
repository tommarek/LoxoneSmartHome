...
import pandas as pd
import pytest
from models.predictors.load_predictor import LoadComponent, LoadPredictor
...
def sample_load_data():
    ...
    outdoor_temp = (
        10 + 8 * np.sin(2 * np.pi * np.arange(672) / 96) + np.random.normal(0, 2, 672)
    )
    ...
    # Total load
    total_load = (
        base_load * daily_pattern * weekend_effect + weather_load + appliance_load
    )
...
class TestLoadPredictor:
    ...
    def test_occupancy_feature_extraction(self, load_config, sample_load_data):
        ...
        occupancy_features = predictor._extract_occupancy_features(
            sample_load_data, time_features
        )
    ...
    def test_load_decomposition(self, load_config, sample_load_data):
        ...
        assert (
            len(decomposed) >= predictor.n_components + 2
        )  # Components + baseline + variable
    ...
    def test_prediction(self, load_config, sample_load_data):
        ...
        predictor.target_scaler.inverse_transform.return_value = np.random.random(
            (100, 1)
        )
    ...
    def test_decomposition_methods(self, load_config, sample_load_data):
        ...
        decomposed_nmf = predictor_nmf._decompose_load(
            sample_load_data["load"], features
        )
        ...
        decomposed_pca = predictor_pca._decompose_load(
            sample_load_data["load"], features
        )
        ...
        decomposed_cluster = predictor_cluster._decompose_load(
            sample_load_data["load"], features
        )
    ...
    def test_feature_configuration_options(self, sample_load_data):
        ...
        features_no_weather = predictor_no_weather._extract_load_features(
            sample_load_data
        )
        ...
        features_no_occupancy = predictor_no_occupancy._extract_load_features(
            sample_load_data
        )
