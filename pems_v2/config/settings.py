...
class InfluxDBSettings(BaseSettings):
    ...
    bucket_prices: str = Field(default="ote_prices", description="Energy price data bucket")
...
class SystemSettings(BaseModel):
    ...
    advisory_mode: bool = Field(default=False, description="Generate recommendations only")
    optimization_interval_seconds: int = Field(default=3600, description="Optimization frequency")
    control_interval_seconds: int = Field(default=300, description="Control update frequency")
    
    @model_validator(mode="after")
    def validate_intervals(self):
        """Validate system timing intervals."""
        if self.control_interval_seconds <= 0:
            raise ValueError(f"Control interval ({self.control_interval_seconds}s) must be positive.")
        
        if self.optimization_interval_seconds <= 0:
            raise ValueError(f"Optimization interval ({self.optimization_interval_seconds}s) must be positive.")
...
class PVModelSettings(BaseModel):
    ...
    confidence_levels: List[float] = Field(description="Confidence levels for uncertainty quantification")
    
    @model_validator(mode="after")
    def validate_prediction_parameters(self):
        """Validate PV prediction parameters."""
        if self.update_interval_seconds < 300:
            raise ValueError(f"PV update interval ({self.update_interval_seconds}s) must be at least 300 seconds.")
        
        if not (1 <= self.horizon_hours <= 168):
            raise ValueError(f"PV prediction horizon ({self.horizon_hours}h) must be between 1 and 168 hours.")
        ...
        if self.confidence_levels != sorted(self.confidence_levels):
            raise ValueError(f"Confidence levels {self.confidence_levels} must be in ascending order.")
...
class RoomSetpoint(BaseModel):
    ...
    def validate_setpoints(self):
        ...
        if not (10.0 <= self.night <= 30.0):
            raise ValueError(f"Night setpoint ({self.night}°C) must be between 10°C and 30°C.")
        
        if not (15.0 <= self.day <= 35.0):
            raise ValueError(f"Day setpoint ({self.day}°C) must be between 15°C and 35°C.")
        
        if self.night > self.day:
            raise ValueError(f"Night setpoint ({self.night}°C) cannot be higher than day setpoint ({self.day}°C).")
...
class ThermalSettings(BaseModel):
    ...
    comfort_band_celsius: float = Field(description="Acceptable temperature deviation in ±°C")
    room_setpoints: Dict[str, RoomSetpoint] = Field(description="Per-room temperature setpoints")
    
    @model_validator(mode="after")
    def validate_comfort_band(self):
        """Validate comfort band is reasonable."""
        if not (0.1 <= self.comfort_band_celsius <= 5.0):
            raise ValueError(f"Comfort band ({self.comfort_band_celsius}°C) must be between 0.1°C and 5.0°C.")
        return self
...
class OptimizationWeights(BaseModel):
    ...
    self_consumption: float = Field(description="PV self-consumption maximization weight")
...
class OptimizationSettings(BaseModel):
    ...
    weights: OptimizationWeights = Field(description="Multi-objective optimization weights")
    ...
    def validate_time_step(self):
        ...
        if not (5 <= self.time_step_minutes <= 120):
            raise ValueError(f"Time step ({self.time_step_minutes} minutes) must be between 5 and 120 minutes.")
        return self
...
class BatterySettings(BaseModel):
    ...
    def validate_efficiency(self):
        ...
        if not (0.5 <= self.efficiency <= 1.0):
            raise ValueError(f"Battery efficiency ({self.efficiency}) must be between 0.5 and 1.0.")
        return self
...
class EVSettings(BaseModel):
    ...
    default_target_soc: float = Field(description="Default target charge level (0.0-1.0)")
    ...
    def validate_ev_parameters(self):
        ...
        if self.battery_capacity_kwh <= 0:
            raise ValueError(f"EV battery capacity ({self.battery_capacity_kwh} kWh) must be positive.")
        
        if not (0.0 < self.default_target_soc <= 1.0):
            raise ValueError(f"EV target SOC ({self.default_target_soc}) must be between 0.0 and 1.0.")
...
class SystemConfig(BaseModel):
    ...
    ev: EVSettings
    room_power_ratings_kw: Dict[str, float] = Field(description="Room power ratings in kW")
