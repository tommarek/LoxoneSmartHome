...
class PVPredictionSettings(BaseSettings):
    ...
    update_interval: int = Field(
        default=3600, description="Prediction update interval in seconds"
    )
...
class ThermalSettings(BaseSettings):
    ...
    comfort_band: float = Field(
        default=0.5, description="Acceptable temperature deviation in ±°C"
    )
...
class OptimizationSettings(BaseSettings):
    ...
    time_step_minutes: int = Field(
        default=60, description="Time discretization in minutes"
    )
    solver: str = Field(
        default="APOPT", description="Mathematical solver (APOPT, IPOPT, etc.)"
    )
    max_solve_time: int = Field(
        default=30, description="Maximum solver time in seconds"
    )

    # Multi-objective optimization weights
    cost_weight: float = Field(
        default=1.0, description="Energy cost minimization weight"
    )
    self_consumption_weight: float = Field(
        default=0.3, description="PV self-consumption maximization weight"
    )
    peak_shaving_weight: float = Field(
        default=0.1, description="Peak demand reduction weight"
    )
    comfort_weight: float = Field(
        default=0.5, description="Thermal comfort maintenance weight"
    )
...
class BatterySettings(BaseSettings):
    ...
    capacity_kwh: float = Field(
        default=10.0, description="Total battery capacity in kWh"
    )
    max_power_kw: float = Field(
        default=5.0, description="Maximum charge/discharge power in kW"
    )
    efficiency: float = Field(
        default=0.95, description="Round-trip efficiency (0.0-1.0)"
    )
...
class EVSettings(BaseSettings):
    ...
    max_power_kw: float = Field(
        default=11.0, description="Maximum EV charging power in kW"
    )
    battery_capacity_kwh: float = Field(
        default=60.0, description="EV battery capacity in kWh"
    )
