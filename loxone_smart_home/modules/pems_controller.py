...
class PEMSController:
    ...
    def __init__(self, mqtt_client, influxdb_client, settings):
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            pems_settings = PEMSControllerSettings(
            ...
            self.pv_predictor = PVPredictor(model_path=self.settings.pv_model_path)
            ...
            try:
                ...
                if result and result.status == "success":
                    ...
                    self.performance_metrics["avg_solve_time_ms"] = (
                        old_avg * (count - 1) + result.solve_time_ms
                    ) / count
                else:
                ...
                self.logger.debug(
                    ...
                    if result
                    else f"Optimization cycle failed in {cycle_time:.1f}s"
                )
        ...
        try:
            ...
            if optimization_result and optimization_result.get("status") == "optimal":
                result = OptimizationResult(
                    ...
                    predicted_comfort_violations=optimization_result.get(
                        "comfort_penalty", 0.0
                    ),
    ...
    async def _generate_predictions(
        ...
        try:
            ...
            return {
                ...
                if not isinstance(pv_forecast, Exception)
                else np.zeros(self.settings.prediction_horizon_hours),
                "thermal_forecast": thermal_forecast
                if not isinstance(thermal_forecast, Exception)
                else {},
                "load_forecast": load_forecast
                if not isinstance(load_forecast, Exception)
                else np.ones(self.settings.prediction_horizon_hours) * 2.0,
            }
        ...
        try:
            ...
            if self.settings.safety_checks:
                if not await self._safety_check(schedule):
                    self.logger.error(
                        "Safety check failed - control schedule not applied"
                    )
                    return
    ...
    async def _safety_check(self, schedule: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if abs(battery_power) > 10000:  # 10kW limit
                self.logger.error(
                    f"Battery power {battery_power}W exceeds safety limit"
                )
                return False
    ...
    async def _publish_status(self, result: OptimizationResult) -> None:
        ...
        try:
            ...
            await self.mqtt_client.publish(
                "pems/status/optimization", json.dumps(status)
            )

            # Publish detailed schedules
            if result.heating_schedule:
                await self.mqtt_client.publish(
                    "pems/status/schedule/heating", json.dumps(result.heating_schedule)
                )

            if result.battery_schedule:
                await self.mqtt_client.publish(
                    "pems/status/schedule/battery", json.dumps(result.battery_schedule)
                )
            ...
            if len(parts) >= 3:
                ...
                if "temperature" in data:
                    ...
                    success = await self.controller.set_room_heating(
                        room, data["heating"], duration_minutes=data.get("duration", 60)
                    )
                    self.logger.info(
            ...
            if len(parts) >= 4:
                ...
                if command == "mode":
                    ...
                    if abs(power_kw) <= 10:  # 10kW limit
                        self.logger.info(
                            f"Battery power control: {power_kw}kW for {duration}min"
                        )
                        # This would be forwarded to the battery controller
                    else:
                        self.logger.error(
                            f"Battery power {power_kw}kW exceeds safety limit"
                        )
